...
function hueRotate(imagedata: ImageData, opts: amount = { amount: 0 }) {
    let i = 0;
    let data = imagedata.data;
    let len = data.length;
    // let opts = opts || { amount: 0 }
    let amount = +opts.amount || 0;
    let r, g, b, a;
    let valueMatric = hueRotateMatrix(amount);
 ...
 */
function hueRotateMatrix(value: number) {
